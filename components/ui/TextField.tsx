  ...
  return (
    <label className={`block ${wrapperClassName}`}>
      ...
      <span className="relative block">
        <input
          {...props}
          className={`h-10 w-full rounded-lg border bg-white px-3 text-sm text-gray-900 outline-none transition placeholder:text-gray-400 focus:ring-4 disabled:bg-gray-50 disabled:text-gray-500 ${
            error
              ? "border-red-300 focus:border-red-500 focus:ring-red-500/10"
              : "border-gray-200 focus:border-brand-500 focus:ring-brand-500/10"
          } ${suffix ? "pe-10" : ""} ${className}`}
        />
        {suffix ? (
