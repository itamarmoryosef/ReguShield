...
-- Email is what actually goes out today; WhatsApp needs a business account we
-- do not have yet. Added separately so existing databases pick it up too.
alter type public.reminder_channel add value if not exists 'email';
...
-- -----------------------------------------------------------------------------
-- scan_quota — one row per account per day
--
-- Scanning bills us per call at OpenAI. Without a ceiling, one account can
-- spend without limit. No policies are granted: RLS is on and nothing matches,
-- so the only way in is the definer function below.
-- -----------------------------------------------------------------------------
...
-- Counts a scan and refuses once the daily allowance is gone.
--
-- The increment and the check are one statement on purpose: reading first and
-- writing second would let two concurrent uploads both see room and both take it.
create or replace function public.consume_scan_quota(p_limit integer default 60)
returns integer
language plpgsql
security definer
set search_path = public
...
begin
  ...
  insert into public.scan_quota as q (user_id, window_date, used)
  values (auth.uid(), current_date, 1)
  on conflict (user_id, window_date)
  do update set used = q.used + 1, updated_at = now()
  returning q.used into v_used;

  if v_used > p_limit then
    raise exception 'scan quota exceeded' using errcode = 'P0001';
  end if;
...
revoke all on function public.consume_scan_quota(integer) from public;
grant execute on function public.consume_scan_quota(integer) to authenticated;
...
alter table public.scan_quota force row level security;
