...
### ואטסאפ: מוכן בקוד, כבוי עד אישור

שכבת ואטסאפ קיימת ומכוסה בבדיקות, אבל דורמנטית. היא נדלקת מהגדרות בלבד —
אין שינוי קוד. ברגע שיש חשבון עסקי מאושר ב-Meta, מגדירים:

| משתנה | מאיפה |
| --- | --- |
| `WHATSAPP_ACCESS_TOKEN` | Meta for Developers → WhatsApp → API Setup |
| `WHATSAPP_PHONE_NUMBER_ID` | אותו מקום, מזהה מספר השולח |
| `WHATSAPP_TEMPLATE_NAME` | שם התבנית שאושרה. ברירת מחדל `document_reminder` |
| `WHATSAPP_TEMPLATE_LANGUAGE` | שפת התבנית. ברירת מחדל `he` |

**המגבלה שחייבים להכיר:** הודעה שהעסק לא יזם חייבת להיות תבנית שאושרה מראש
ב-Meta. טקסט חופשי מותר רק בחלון של 24 שעות מאז שהלקוח כתב אלינו, ותזכורת
מתוזמנת מעולם לא נמצאת בו. לכן צריך ליצור תבנית בקונסולה עם ארבעה פרמטרים
בסדר הזה:
...
כלומר: שם בעל העסק, שם העסק, מספר המסמכים, ורשימת השמות.

בחירת הערוץ: אם ואטסאפ מוגדר ולעסק יש נייד תקין — ההודעה נשלחת בוואטסאפ. כשל
בוואטסאפ נופל חזרה למייל, כי תזכורת שהגיעה בערוץ פחות נוח עדיפה על תזכורת
שנעלמה. מספר נייד נגזר מ-`mobile` ואז מ-`phone`, מנורמל ל-E.164, וקווי נייח
נדחה — הוא לא יכול לקבל ואטסאפ, ושליחה אליו הייתה מדווחת הצלחה שלא קרתה.

### תקופת צינון

ה-cron רץ כל יום, אבל מסמך לא מקבל תזכורת כל יום. אחרי שליחה הוא שותק לפי
דחיפות: 7 ימים אם פג או פג בתוך שבוע, 14 ימים עד חודש, 30 ימים מעל זה.
בלי זה מסמך שפג תוקפו היה מייצר מייל זהה כל בוקר עד שמישהו יטפל בו.
...
התבניות בנויות כמסמך HTML מלא עם הצהרת קידוד ופריסת טבלאות, ולא כ-div מסוגנן.
זה לא עניין של טעם: לקוחות דואר, ובראשם Gmail, מסננים סימון שאינו במבנה
המקובל לדואר ועלולים להציג הודעה ריקה. מאותה סיבה הכתובת מופיעה גם כטקסט גלוי
מתחת לכפתור, כדי שגם אם הכפתור נופל נשאר משהו ללחוץ עליו.

**מלכודת תפעולית:** לאחר עדכון תבנית, Supabase ממשיך לשלוח את הגרסה הקודמת
מהזיכרון במשך שתיים-שלוש דקות. בדיקה מיד לאחר העדכון תראה את התבנית הישנה
ותיראה ככשל. יש להמתין ולשלוח שוב, ולאמת מול גוף ההודעה שנשלחה בפועל ולא רק
מול מה שמאוחסן בהגדרות.
